...
const GetCharity: React.FC = () => {
  ...
  const deleteCharity = async (id: number) => {
    try {
      await axios.delete(`https://backend-kindr.onrender.com/${id}`);
      setCharities(charities.filter((charity) => charity.id !== id));
    } catch (err) {
      setError("Failed to delete charity. Please try again later.");
            ...
            <ScrollArea className="w-[95vh] xl:w-full whitespace-nowrap rounded-md z-10">
              <div className="flex relative gap-6">
                {charities.map((charity) => (
                  ...
                  >
                    <img
                      src={`https://backend-kindr.onrender.com/${charity.image}`}
                    ...
                    <div className="relative -top-5 p-4 mx-3 bg-secondary z-10 border-white/50 border-2">
                      ...
                      <div className="flex flex-row items-center justify-between mt-4">
                        <a
                          href={`https://backend-kindr.onrender.com/${charity.document}`}
                          target="_blank"
